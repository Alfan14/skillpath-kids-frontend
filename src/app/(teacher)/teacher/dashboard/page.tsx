...
export default function TeacherDashboardPage() {
  return (
    <div className="flex flex-col gap-6">
      <section className="overflow-hidden rounded-[28px] border border-[#d4e3ff] bg-[#d4e3ff] p-5 shadow-[0_16px_40px_rgba(0,72,131,0.10)] md:p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            ...
            <Link
              href="/teacher/assessment"
              className="mt-5 inline-flex items-center justify-center gap-2 rounded-[18px] border border-[#e8c426] bg-[#ffe173] px-5 py-3 text-sm font-black text-[#0f1d24] shadow-[0_5px_0_0_#d9b739] transition-all hover:translate-y-[1px] hover:bg-[#ffd84d]"
            >
              Mulai Assessment
          ...
          <Image
            ...
            height={360}
            priority
            className="teacher-float mx-auto h-auto w-full max-w-[240px] shrink-0 motion-reduce:animate-none md:max-w-[360px] lg:mx-0 lg:max-w-[430px]"
          />
        </div>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {stats.map(({ label, value, icon: Icon, bg, text }) => (
          <div
            key={label}
            className="rounded-[18px] border border-[#d4e3ff] bg-white p-5 shadow-[0_10px_28px_rgba(0,72,131,0.06)] transition-all duration-200 hover:scale-[1.01] motion-reduce:transition-none motion-reduce:hover:scale-100"
          >
      ...
      <section className="grid gap-4 md:grid-cols-2">
        {quickLinks.map(({ title, description, href, image, icon: Icon, badge }) => (
          <Link
            key={href}
            href={href}
            className="group overflow-hidden rounded-[22px] border border-[#d4e3ff] bg-white p-5 shadow-[0_12px_32px_rgba(0,72,131,0.08)] transition-all duration-200 hover:scale-[1.01] hover:shadow-[0_16px_40px_rgba(0,72,131,0.12)] motion-reduce:transition-none motion-reduce:hover:scale-100"
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <Image
                ...
                width={120}
                height={100}
                className="h-auto w-full max-w-[92px] shrink-0 transition-transform duration-300 group-hover:-translate-y-1 motion-reduce:transition-none sm:max-w-[112px]"
              />
            </div>
          </Link>
        ))}
      ...
      <section className="rounded-[22px] border border-[#d4e3ff] bg-white p-5 shadow-[0_12px_32px_rgba(0,72,131,0.06)]">
